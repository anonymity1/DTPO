# ip_model_whole(vectorPenalty).py compute the gradient of all G coveringLP
...
# 这段300行的代码主要是用来确保输入参数符合特定的格式和类型，并且没有无效或不适当的值，如 inf、nan 或 None。
...
# 这段代码100多行,定义了一个函数 postsolve，
# 其主要作用是将标准形式线性规划问题的解转换回原始问题的解，
# 并计算原始问题的目标函数值、上界约束的松弛变量和等式约束的残差。
...
# 这段代码长度将近100行,
# 其目的是返回一个适合特定线性系统的求解器函数。
# 这个函数可以处理不同类型的矩阵（稀疏或密集），
# 并且根据系统的特性选择合适的求解方法。
...
# 代码长度300行,包括识别问题中的冗余约束,无效约束,以及固定变量等
...
# 分析AbGh的代码，在一个优化问题或线性/二次规划问题中，
# A 和 G 可能分别代表不同类型的约束矩阵，而 b 和 h 是对应的约束向量。
# 这个断言确保了至少有一组有效的约束被提供给问题。
...
# 这段代码是一个线性规划问题的后处理（postprocessing）函数，
# 它用于将预处理（presolve）步骤中所作的变换还原回来。
# 在预处理阶段，可能会移除一些变量或对问题进行变换以简化求解过程。
# 一旦找到了简化问题的解，就需要通过后处理将这个解转换回原始问题的上下文中。
...
# c是cx中的c，Ax=b，Gx < h
def IPOfunc(A =None,b =None,h=None,c=None,GTrue=None,penalty=None,alpha0=0.9995,beta=0.1,pc = True,
    ...
    class IPOfunc_cls(Function):        
        ...
        def forward(ctx,G):
            ...
            nonlocal run_time
            start = time.time()
            # detach()函数创建新的张量副本，但是不记录梯度信息
            # numpy()方法将torch张量转化为numpy数组
            c_=c.detach().numpy()
            if b is not None:
                b_=b.detach().numpy()
                if b_.shape[0] ==0:
            ...
            try:
                c_, G_, h_,A_, b_,bounds_, x0,n_x,n_eq,n_ub = _preprocess(c_, A_, b_, G_, 
                    h_,bounds_ )
                # my_print(
                #     "After pre process shape of c {} of A{} b{} h{} G{}".format(
                #         c_.shape,A_.shape,b_.shape,h_.shape,G_.shape
                #     ),
                #     c_.shape
                # )
            ...
            if presolve:
                c_, c0, G_, h_, A_, b_, bounds_, x, x0, undo, complete, status, message = _presolve(
                    c_, G_, h_,A_, b_,bounds_, x0, rr= A_eq_rr, tol=1e-9
                )

            # 完成presolve和preprocess
            ...
            if not complete:
                ...
                if kappa>1e-3:
                    logging.info("kappa >0 ; c.x {}, b.y {}".format(c_.dot(x),b_.dot(y)))
                ...
                global x_s1
                x_s1 = torch.from_numpy(x_sol).float()
                
                # Stage 2:
                m2 = gp.Model()
                m2.setParam('OutputFlag', 0)
                x_m2 = m2.addVars(varNum, vtype=GRB.CONTINUOUS, name='x')   # x_(1)
                y_m2 = m2.addVars(varNum, vtype=GRB.CONTINUOUS, name='y')   # sigma
                
                rowSizeG = GTrue.shape[0]
                c_list = c.tolist()
                G_positive = -GTrue
                h_positive = -h
                G_list = G_positive.tolist()
                penalty_numpy  = penalty.numpy()
                h_numpy = h_positive.numpy()
                OBJ = x_m2.prod(c_list)
                for i in range(varNum):
                    OBJ = OBJ + (1 + penalty_numpy[i]) * c_list[i] * y_m2[i]
                m2.setObjective(OBJ, GRB.MINIMIZE)
                
                for i in range(rowSizeG):
                    m2.addConstr((x_m2.prod(G_list[i]) + y_m2.prod(G_list[i])) >= h_numpy[i])
                for i in range(varNum):
                    m2.addConstr(x_m2[i] == x_sol[i])
...
#                        torch.from_numpy(s_sol),
        ...
        def backward(ctx,del_x):
            ...
            nonlocal run_time
            start = time.time()
            x,y,t,tau,kappa,c,A,b,trueA,trueb,trueG,trueh,x_sol,t_sol = ctx.saved_tensors
            ...
            fGx = np.zeros((n_var, rowSizeG, n_var))

            for j in range(n_var):
                matTemp = mu * x_numpy[j] * G_numpy.T.dot(hGx2)
                matTemp[j,:] = matTemp[j,:] + mu / hGx
                fGx[j] = matTemp.T
            ...
            A = A[:, :colSizeG]
            newX = torch.cat((x_sol, t_sol), 0)
            rowSizeG = A.size(dim=0)
            zeroMatrix = torch.zeros((rowSizeG, colSizeG))
            newA1 = torch.cat((A, zeroMatrix), 1)
            identityMatrix = torch.eye(colSizeG)
            newA2 = torch.cat((identityMatrix, -1*identityMatrix), 1)
            newA = torch.cat((newA1, newA2), 0)
            newb = torch.cat((b, x_s1), 0)
            newA = newA.numpy()
            newb = newb.numpy()
            newX = newX.numpy()
            newH = np.zeros((2*colSizeG, 2*colSizeG))
            for i in range(2*colSizeG):
                if newX[i] != 0:
                    newH[i][i] = mu / (newX[i]*newX[i])
                else:
                    newH[i][i] = 0
            if np.linalg.det(newH) == 0:
                newH = newH + damping*np.eye(2*colSizeG)
            ...
            try:
              ...
              newAHA_inverse = np.eye(rowSizeG+colSizeG)
            identityMatrix1 = np.eye(rowSizeG+colSizeG)
            dx2b = newH.dot(newA.T).dot(newAHA_inverse).dot(identityMatrix1)
            zeroMatrix2 = np.zeros((rowSizeG, colSizeG))
            dbxs1 = np.concatenate((zeroMatrix2, identityMatrix), 0)
            zeroMatrix3 = np.zeros((colSizeG, colSizeG))
            ...
            for i in range(colSizeG):
                dRxs2[i] = c[i] + penalty[i] * c[i]
                dRxs1[i] = -penalty[i] * c[i]

            Grad1 = dRxs2.dot(dxs2xs1)
            for i in range(n_var):
                dxG[..., i] = np.dot(Grad1, dxG[..., i]) + np.dot(dRxs1, dxG[..., i])
